...
`sonar_fix()` groups corrections into 18 categories. Use the `fixes` parameter to select which ones to apply.
...
- Skips variables shorter than 3 characters
- Skips common names (`i`, `j`, `tmp`, etc.)
- Skips assignments from function calls (may have side effects)
- Skips `<<-` super-assignments (visible elsewhere)

⚠️ **Enabled in `fixes = "all"`** — use with `dry_run = TRUE` first to review what would be removed.

### Duplicate Libraries (`"duplicate_libs"`)

Detects and removes duplicate `library()` calls loading the same package multiple times:
...
- **`"library"`** — Detects `library(x)` calls that may be unused (also counts duplicates)
